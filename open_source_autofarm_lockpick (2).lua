...
local function readCashAmountValue()
    ...
    if not object then return StatsInfo.CashAmount end
    ...
    if ok then
        StatsInfo.CashAmount = parseCashTextToNumber(value)
    end
...
local function readStatsGui()
    ...
    StatsInfo.AllowanceAmount = parseNumber(allowance) or StatsInfo.AllowanceAmount
    StatsInfo.BankAmount = parseNumber(bank) or StatsInfo.BankAmount
end
...
local function performDepositRequest(events, cash)
    ...
    if not MoveToTarget(atmMainPart) then
        return false
    end
...
local function MainFarmLoop()
    ...
    while true do
        ...
        readStatsGui()
        readCashAmountValue()
        if Settings.AutoAllowance and StatsInfo.AllowanceAmount and StatsInfo.AllowanceAmount <= 0 then
...
task.spawn(function()
    while true do
        ...
        allowancePara:SetDesc("$" .. math.floor(StatsInfo.AllowanceAmount))
        task.wait(0.5)
    end
end)
