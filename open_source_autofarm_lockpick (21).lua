...
local function HackSafeWithLockpick(safeObj)
    ...
    local consecutiveSameFailure = 0
    ...
    while Settings.Enabled and safeObj and safeObj.Parent do
        ...
        if reason ~= "lockpick_success" and reason == lastFailureReason then
            consecutiveSameFailure = consecutiveSameFailure + 1
        else
            consecutiveSameFailure = 1
        end
        lastFailureReason = reason

        -- Одна и та же причина провала повторилась дважды подряд - это не
        -- проблема конкретного лок-пика (его не помечаем как "плохой"), а
        -- признак того, что цель уже недоступна (сломана кем-то другим,
        -- сервер отклоняет и т.д.). Не перебираем весь инвентарь и не
        -- докупаем новые лок-пики - сразу сдаёмся.
        if not opened and consecutiveSameFailure >= 2 then
            Log("Причина провала повторилась дважды подряд (" .. tostring(reason) .. ") - похоже цель уже недоступна, бросаю без докупки")
