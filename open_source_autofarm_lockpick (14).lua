...
local BufferZones = {
    {
        name = "Burmalda",
        token = "ho",
        waypoint1 = Vector3.new(-4381.93, 9.15, -59.13),
        waypoint2 = Vector3.new(-4382.03, 25.48, -58.07),
    },
}

local function getBufferZoneForTarget(targetName)
    local nameLower = tostring(targetName or ""):lower()
    for _, zone in ipairs(BufferZones) do
        if nameLower:find("%f[%a]" .. zone.token .. "%f[%A]") then
            return zone
        end
    end
    return nil
...
local function MoveToTargetSmart(targetPart, targetObj, kind)
    ...
    Log("Буферная зона " .. zone.name .. ": иду к вейпоинту 1")
...
local function startLockpickCameraLock(part)
    ...
    LockpickCameraLockConnection = RunService.RenderStepped:Connect(function()
        ...
        local character = LocalPlayer.Character
        local hrp = character and character:FindFirstChild("HumanoidRootPart")
        local eyePos = hrp and (hrp.Position + Vector3.new(0, 2, 0)) or cam.CFrame.Position
        cam.CFrame = CFrame.new(eyePos, part.Position)
    end)
end
...
local function tryLockpickTargetViaRemote(target, tool)
    ...
    local clicked = clickToOpenLockpickMinigame(target)
    if not clicked then
        Log("Не удалось прицелиться и кликнуть по сейфу")
    end
    task.wait(0.15)

    local gui = waitForLockpickGui(2)
    if not gui and not clicked then
        -- Вторая попытка: возможно первая камера/клик не совпали
        clickToOpenLockpickMinigame(target)
        gui = waitForLockpickGui(2)
    end
    if gui then
        Log("LockpickGUI найден и увеличен")
    else
        Log("LockpickGUI не появился, пропускаю попытку без слепого инвока")
        return false, "lockpick_gui_not_found"
    end
...
local function HackSafeWithLockpick(safeObj)
    ...
    local consecutiveGuiNotFound = 0
    local abandonedAsUnavailable = false
    ...
    while Settings.Enabled and safeObj and safeObj.Parent do
        ...
        if tick() - startTime > HACK_TIME_BUDGET then
            Log("Стою у цели дольше " .. HACK_TIME_BUDGET .. " сек, меняю таргет")
            break
        end
        ...
        local remainingBudget = HACK_TIME_BUDGET - (tick() - startTime)
        if remainingBudget <= 0 then break end

        local opened, reason = tryLockpickTargetViaRemote(safeObj, tool)
        Log("Попытка лок-пика (" .. tool:GetDebugId() .. "): " .. tostring(reason))

        if reason == "lockpick_gui_not_found" then
            consecutiveGuiNotFound = consecutiveGuiNotFound + 1
        else
            consecutiveGuiNotFound = 0
        end

        -- GUI дважды подряд не появился, хотя клик реально прицелился и
        -- сработал - это не проблема конкретного лок-пика (его не помечаем
        -- как "плохой"), а признак того, что серверу нечего показывать:
        -- сейф, скорее всего, уже открыт кем-то другим или недоступен.
        -- Не перебираем весь инвентарь и не докупаем новые - сразу сдаёмся.
        if consecutiveGuiNotFound >= 2 then
            Log("GUI не появляется дважды подряд - похоже цель уже недоступна, бросаю без докупки")
            abandonedAsUnavailable = true
            break
        end
        ...
        if opened then
            ...
            while safeObj.Parent
                ...
                and tick() - completedAt < math.min(4, HACK_TIME_BUDGET - (tick() - startTime))
            do
                task.wait(0.10)
            end
            break
    ...
    if not ok then
        Log("Сейф так и не открылся лок-пиком")
    end

    if not ok and abandonedAsUnavailable then
