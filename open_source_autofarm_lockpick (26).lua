...
local function MoveToTargetSmart(targetPart, targetObj, kind)
    ...
    -- просто идём напрямую к цели обычным умным путём.
    local character = LocalPlayer.Character
    local hrp = character and character:FindFirstChild("HumanoidRootPart")
    local ZONE_ALREADY_INSIDE_RADIUS = 60

    if hrp and (hrp.Position - zone.waypoint2).Magnitude <= ZONE_ALREADY_INSIDE_RADIUS then
        Log("Уже внутри зоны " .. zone.name .. ", иду к " .. tostring(targetObj and targetObj.Name) .. " напрямую")
...
local function HackSafeWithLockpick(safeObj)
    ...
    local HACK_TIME_BUDGET = 5
    local startTime = tick()
    local triedTools = {}

    -- Независимый сторожевой таймер: 5-4-3-2-1 в лог + жёсткий флаг отказа
    -- по истечении. Не зависит от анализа причин провала конкретных попыток
    -- (то оказалось ненадёжным) - просто гарантированный хард-таймаут.
    local forceAbandon = false
    task.spawn(function()
        for i = HACK_TIME_BUDGET, 1, -1 do
            if forceAbandon then return end
            Log("Взлом лок-пиком: " .. i .. "...")
            StatusText = "Взлом лок-пиком: " .. i .. " сек"
            task.wait(1)
        end
    end)
    task.delay(HACK_TIME_BUDGET, function()
        forceAbandon = true
    end)
    ...
    while Settings.Enabled and safeObj and safeObj.Parent and not forceAbandon do
        ...
        if forceAbandon then break end
        ...
        if opened then
            ...
            while safeObj.Parent
                ...
                and not forceAbandon
                and tick() - completedAt < 4
            do
                task.wait(0.10)
            end
            break
