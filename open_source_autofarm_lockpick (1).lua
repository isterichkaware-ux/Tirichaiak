...
local function MoveToTarget(targetPart, targetObj)
    RiseToTargetY()
    local character = LocalPlayer.Character
...
-- Минигра лок-пика: LockpickGUI появляется в PlayerGui в момент открытия сейфа.
-- Внутри: MF.LP_Frame.Frames.{B1,B2,B3}.Bar.UIScale (масштаб зон попадания)
-- и MF.LP_Frame.Line (бегущая полоска), а прогресс считается по Uses/MaxValue
-- на самом туле Lockpick.

local LOCKPICK_ZONE_NAMES = { "B1", "B2", "B3" }
local LOCKPICK_ZONE_SCALE = 10

local function scaleLockpickZones(lockpickGui)
    if not lockpickGui then return false end
    ...
    for _, zoneName in ipairs(LOCKPICK_ZONE_NAMES) do
        local zone = frames:FindFirstChild(zoneName)
        local bar = zone and zone:FindFirstChild("Bar")
        local uiScale = bar and bar:FindFirstChild("UIScale")
        if uiScale then
            uiScale.Scale = LOCKPICK_ZONE_SCALE
            scaledAny = true
        end
    end
    return scaledAny
...
-- Постоянный наблюдатель: как только LockpickGUI появляется в PlayerGui (в
-- любой момент, от любого сейфа), сразу увеличиваем зоны. Это подстраховка
-- на случай, если основной цикл ещё не успел среагировать.
...
local function tryLockpickTargetViaRemote(target, tool)
    ...
    if gui then
        ...
        Log("LockpickGUI не появился за 2 сек, всё равно пробую взломать")
    end
    ...
    while Settings.Enabled and safeObj and safeObj.Parent do
        ...
        if CountTools("Lockpick") == 0 then
            ...
            if safeObj.Parent and not (broken and broken.Value) then
                MoveToTarget(safeObj:FindFirstChild("MainPart") or safeObj.PrimaryPart)
            end
        end

        local tool, uses = getLockpickToolState()
        if not tool then
            Log("Лок-пик не найден в инвентаре")
            task.wait(0.5)
            continue
        end
...
local function MainFarmLoop()
    ...
    while true do
        ...
        local moveSuccess, moveReason = MoveToTarget(mainPart, nextTarget)
        ...
        if moveSuccess then
            ...
            elseif Settings.BreakMethod == "Lockpick" then
                hackSuccess = HackSafeWithLockpick(nextTarget)
            else
                if not LocalPlayer.Character:FindFirstChild("Crowbar") then
                    EquipTool("Crowbar")
                end
